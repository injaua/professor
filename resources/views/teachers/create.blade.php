        ...
        <form action="{{ route('teachers.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" id="name"
                       class="block w-full border-2 mt-2 border-blue-500 rounded-md bg-white px-4 py-2 text-base text-gray-900 outline-none placeholder:text-gray-400 focus:border-green-500 focus:ring-2 focus:ring-green-500 focus:outline-none sm:text-sm"
            ...
            <div>
                ...
                <select name="classroom[]" id="classroom"
                    ...
                    @foreach ($classroom as $class)
                        <option value="{{ $class->id }}" {{ in_array($class->id, old('classroom', [])) ? 'selected' : '' }}>
                            {{ $class->nr_sala }}
                        </option>
                    @endforeach
                </select>
